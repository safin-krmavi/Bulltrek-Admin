...
export default function SmartGrid() {
  ...
  useEffect(() => {
    async function fetchStrategyDetails() {
      ...
      try {
        const response = await apiClient.get('/api/v1/strategies/2')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.post('/api/v1/strategies/2/run', {
        config: formData
      })
  ...
  return (
    <div className="w-full max-w-xl rounded-full mx-auto">
      ...
      <StrategyResponsePopup
        ...
        isSuccess={isSuccess}
        strategyName={strategyDetails?.name || "Smart Grid Strategy"}
      />
    </div>
  )
}
